...
our %async_run_methods = (
        'filter_contigs_async' => 'Muscle.filter_contigs',
);

our %async_check_methods = (
        'filter_contigs_check' => 'Muscle.filter_contigs',
);
...
sub call_method {
    ...
    my $ctx = Muscle::MuscleServerContext->new($self->{loggers}->{userlog},
...
{
    # Service Muscle requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for Muscle but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = Muscle::MuscleServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service Muscle.");
    }
...
package Muscle::MuscleServerContext;

use strict;

=head1 NAME

Muscle::MuscleServerContext
...
package Muscle::MuscleServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = Muscle::MuscleServer->new(
